...
trait CoffeeShop[F[_]] {
  def types: F[Array[String]]

  def drinks(coffeeType: String): F[Array[Drink]]
}
...
object CoffeeShop {
  ...
  def impl[F[_] : Concurrent](C: Client[F]): CoffeeShop[F] = new CoffeeShop[F] {
    ...
    def types: F[Array[String]] = {
      C.expect[Array[String]](GET(Uri.unsafeFromString(endpoint)))
        .adaptError { case t => CoffeeShopError(t) }
    }

    def drinks(coffeeType: String): F[Array[Drink]] = {
      C.expect[Array[Drink]](GET(Uri.unsafeFromString(s"$endpoint/$coffeeType/")))
        .adaptError { case t => CoffeeShopError(t) }
    }
  }
}
