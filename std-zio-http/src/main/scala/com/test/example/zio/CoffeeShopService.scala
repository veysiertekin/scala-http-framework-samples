...
case class CoffeeShopLive() extends CoffeeShopService {
  ...
  private val endpoint = "https://protected-dawn-85371.herokuapp.com/coffee-shop/drinks"
  ...
  private def invokeGetRequest[T: JsonDecoder](path: String): IO[Throwable, T] = {
    for {
      ...
      response <- Client.request(req, ClientSSLOptions.DefaultSSL).provideLayer(env)
